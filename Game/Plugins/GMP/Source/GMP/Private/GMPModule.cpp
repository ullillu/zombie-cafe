...
{
	// Kombez team fix
	const auto NormalizeTypeName = [](FName InTypeName)
	{
		const FString TypeName = InTypeName.ToString();
		if (TypeName.StartsWith(TEXT("/Script/")))
		{
			int32 DotIndex = INDEX_NONE;
			if (TypeName.FindLastChar(TEXT('.'), DotIndex) && (DotIndex + 1) < TypeName.Len())
			{
				return FName(*TypeName.Mid(DotIndex + 1));
			}
		}
	...
	ParentType = NormalizeTypeName(ParentType);
	//~Kombez team fix
	...
	// Kombez team fix
	// Fall back to resolving both names to UClass so native short names and full path names
	// are still considered compatible in non-editor targets such as dedicated server.
	const auto ResolveClass = [](FName InTypeName) -> UClass*
	{
		const FString TypeName = InTypeName.ToString();

		if (UClass* DynamicType = Reflection::DynamicClass(TypeName))
		{
			return DynamicType;
		}

		if (UClass* FoundClass = FindObject<UClass>(nullptr, *TypeName))
		{
			return FoundClass;
		}

		if (UClass* LoadedClass = LoadObject<UClass>(nullptr, *TypeName))
		{
			return LoadedClass;
		}

		FSoftClassPath SoftClassPath(TypeName);
	...
	if (UClass* TypeClass = ResolveClass(Type))
	{
		if (UClass* ParentClass = ResolveClass(ParentType))
		{
			return TypeClass->IsChildOf(ParentClass);
		}
	}
	//~Kombez team fix
	return false;
