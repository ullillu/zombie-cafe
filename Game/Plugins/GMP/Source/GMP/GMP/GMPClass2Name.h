...
#define GMP_MANUAL_GENERATE_NAME_FULL(NAME)                                                \
	...
	*/
	...
	template<typename T, typename DecayedType = std::remove_cv_t<std::remove_reference_t<T>>>
	struct TInterfaceParamConvert
	{
		using Type = std::conditional_t<is_native_inc_v<T>, native_inc_to_struct<T>, typename std::add_lvalue_reference<T>::type>;
	};
#if UE_5_00_OR_LATER
	template<typename T, typename PointeeType>
	struct TInterfaceParamConvert<T, TObjectPtr<PointeeType>>
	{
		using Type = PointeeType*;
	};
#endif
	template<typename T>
	using InterfaceParamConvert = typename TInterfaceParamConvert<T>::Type;
	template<typename T>
